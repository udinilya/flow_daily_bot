...
def status_reminders_are_running(context: CallbackContext):
    with open('chat_list.txt', 'r', encoding='utf8') as f:
...
def add_chat_id_in_chat_list(update: Update, context: CallbackContext):
    ...
    if current_chat not in chat_list:
        with open('chat_list.txt', 'a', encoding='utf8') as f:
            print(current_chat, file=f)


def get_chat_members(update: Update, context: CallbackContext):
    with open(f'chat_members{update.message.chat_id}.txt', 'a+', encoding='utf8') as f:
        print(update.effective_user.name, file=f)


def get_responded_members(update: Update, context: CallbackContext):
    with open(f'responded_members{update.message.chat_id}.txt', 'a+',
              encoding='utf8') as f:
        print(update.effective_user.name, file=f)

    def remind_about_missed_persons(context: CallbackContext):
        with open(f'responded_members{update.message.chat_id}.txt', 'r',
                  encoding='utf8') as f:
            respond_list = f.readlines()

        with open(f'chat_members{update.message.chat_id}.txt', 'r',
                  encoding='utf8') as f:
            chat_list = f.readlines()
    ...
    context.job_queue.run_daily(remind_about_missed_persons, time=datetime.time(16, 29,
                                tzinfo=pytz.timezone('Europe/Moscow')), days=tuple(range(0, 5)))


persistence = PicklePersistence(filename='persistent_storage.pkl')

updater = Updater('2113078267:AAHVEPRb8zj-K-vLVewkJgtiVpMQeiIoVng', persistence=persistence, use_context=True)
...
updater.job_queue.run_daily(status_reminders_are_running, time=datetime.time(16, 24, tzinfo=pytz.timezone('Europe/Moscow')),
                            days=tuple(range(0, 5)))
updater.job_queue.run_daily(remind_about_status, time=datetime.time(16, 25, tzinfo=pytz.timezone('Europe/Moscow')),
                            days=tuple(range(0, 5)))
